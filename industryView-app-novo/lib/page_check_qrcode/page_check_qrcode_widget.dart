...
class _PageCheckQrcodeWidgetState extends State<PageCheckQrcodeWidget> {
      ...
      try {
        ...
        if ((_model.validToken?.succeeded ?? true)) {
          ...
          if (kDebugMode) {
            print('=== QR PAGE DEBUG ===');
            print(
                'tokenProjectId (appState): ${AppState().user.projectId}, meCall: ${meCall.projectId(tokenJson)}');
            print(
                'tokenTeamsId (appState): ${AppState().user.teamsId}, meCall: ${meCall.teamsId(tokenJson)}');
            print('====================');
          }
          ...
          if (existingScheduleId != 0 &&
            ...
            AppState()
                .updateUserStruct((e) => e..sheduleId = existingScheduleId);
  ...
  void _updateUserFromToken(dynamic tokenJson) {
    final meCall =
        AuthenticationGroup.getTheRecordBelongingToTheAuthenticationTokenCall;
    AppState().updateUserStruct(
      (e) => e
        ...
        ..name =
            valueOrDefault<String>(meCall.name(tokenJson), AppState().user.name)
        ..email = valueOrDefault<String>(
            meCall.email(tokenJson), AppState().user.email)
        ..phone = valueOrDefault<String>(
            meCall.phone(tokenJson), AppState().user.phone)
        ..image = valueOrDefault<String>(
            meCall.image(tokenJson), AppState().user.image)
        ..teamsId = valueOrDefault<int>(
            meCall.teamsId(tokenJson), AppState().user.teamsId)
        ..projectId = valueOrDefault<int>(
            meCall.projectId(tokenJson), AppState().user.projectId)
        ..companyId = valueOrDefault<int>(
            meCall.companyID(tokenJson), AppState().user.companyId)
        ..sprint = SprintsStruct(
          id: valueOrDefault<int>(
              meCall.sprintId(tokenJson), AppState().user.sprint.id),
          title: valueOrDefault<String>(
              meCall.spritnTitle(tokenJson), AppState().user.sprint.title),
          objective: valueOrDefault<String>(meCall.sprintObjective(tokenJson),
              AppState().user.sprint.objective),
          startDate: valueOrDefault<int>(meCall.sprintDtStart(tokenJson),
              AppState().user.sprint.startDate),
          endDate: valueOrDefault<int>(
              meCall.sprintDtEnd(tokenJson), AppState().user.sprint.endDate),
          progressPercentage: valueOrDefault<double>(
              meCall.sprintProgress(tokenJson),
              AppState().user.sprint.progressPercentage),
          tasksConcluidas: valueOrDefault<int>(
              meCall.concluidas(tokenJson)?.length,
              AppState().user.sprint.tasksConcluidas),
          tasksAndamento: valueOrDefault<int>(
              meCall.andamento(tokenJson)?.length,
  ...
  Future<void> _saveScheduleToBackend() async {
    if (kDebugMode) {
      ...
      print(
          'projectId: ${AppState().user.projectId}, teamsId: ${AppState().user.teamsId}');
    }
    ...
    try {
      ...
      if (currentScheduleId != 0) {
        if (kDebugMode) {
          print('UPDATING current session schedule $currentScheduleId');
        }
        ...
        final response =
            await ProjectsGroup.adicionaColaboradoresNaEscalaCall.call(
          teamsId: AppState().user.teamsId,
          usersIdList: AppState().allIds,
          projectsId: AppState().user.projectId,
        ...
        if (kDebugMode) {
          print(
              'CREATE response succeeded: ${response.succeeded}, statusCode: ${response.statusCode}');
        ...
        if (response.succeeded) {
          ...
          if (newScheduleId != null) {
            ...
            if (kDebugMode) {
              print('SAVE: novo scheduleId salvo no AppState: $newScheduleId');
            }
          }
        }
      }
                                          ...
                                          child: AppButton(
                                            onPressed: () async {
                                              ...
                                              if (membrosList != null) {
                                                ...
                                                for (final membro
                                                    in membrosList) {
                                                  final matricula = castToType<
                                                          String>(
                                                      getJsonField(membro,
                                                          r'''$.users.hr_data.matricula'''));
                                                  if (matricula != null &&
                                                      matricula ==
                                                          _model.qrcode) {
                                              ...
                                              if (!foundLocally) {
                                                ...
                                                shouldSetState = true;
                                                qrUserId = ReportsGroup
                                                    .qrcodeReaderCall
                                                    .userId((_model
                                                        .apiQrcode?.jsonBody));
                                                qrUserName = ReportsGroup
                                                    .qrcodeReaderCall
                                                    .userName((_model
                                                        .apiQrcode?.jsonBody));
                                                          ...
                                                          child:
                                                            ...
                                                            description:
                                                                AppLocalizations.of(
                                                                    ...
                                                                    .getVariableText(
                                                              ptText:
                                                                  'O funcionário${qrUserName != null && qrUserName.isNotEmpty ? " $qrUserName" : ""} não pertence a esta equipe.',
                                                              esText:
                                                                  'El empleado${qrUserName != null && qrUserName.isNotEmpty ? " $qrUserName" : ""} no pertenece a este equipo.',
                                                              enText:
                                                                  'The employee${qrUserName != null && qrUserName.isNotEmpty ? " $qrUserName" : ""} does not belong to this team.',
                                                            ),
                                                          ),
                                                        );
                                                      },
                                              ...
                                              if (qrUserId != null) {
                                                        ...
                                                        child: ModalInfoWidget(
                                                          ...
                                                          description:
                                                              AppLocalizations.of(
                                                                  ...
                                                                  .getVariableText(
                                                            ptText:
                                                                '${qrUserName ?? "Funcionário"} já foi registrado hoje.$entryTimeText',
                                                            esText:
                                                                '${qrUserName ?? "Empleado"} ya fue registrado hoy.$entryTimeText',
                                                            enText:
                                                                '${qrUserName ?? "Employee"} has already been registered today.$entryTimeText',
                                                          ),
                                                        ),
                                                      );
                                                    },
                                                ...
                                                AppState().update(() {
                                                  AppState().addToQrCodeIDs(
                                                      qrUserId!);
                                                  AppState()
                                                      .addToAllIds(qrUserId);
                                                  // Armazenar horário de entrada local
                                                  AppState().addEscalaEntryTime(
                                                      qrUserId,
                                                      DateTime.now()
                                                          .toUtc()
                                                          .toIso8601String());
                                                      ...
                                                      child: GestureDetector(
                                                        ...
                                                        child: ModalInfoWidget(
                                                          ...
                                                          description:
                                                            ...
                                                            esText:
                                                                'Este usuario no tiene acceso o no está registrado',
                                                            enText:
                                                                'This user does not have access or is not registered',
                                                          ),
                                                        ),
                                                      ),
                                                    );
                                            ...
                                            child: AppButton(
                                              onPressed: !(AppState()
                                                  ...
                                                  : () async {
                                                      ...
                                                      final ctxIdx = AppState()
                                                          .addOrGetProjectContext(
                                                        projectId: AppState()
                                                            .user
                                                            .projectId,
                                                        projectName: AppState()
                                                            .user
                                                            .projectName,
                                                        teamsId: AppState()
                                                            .user
